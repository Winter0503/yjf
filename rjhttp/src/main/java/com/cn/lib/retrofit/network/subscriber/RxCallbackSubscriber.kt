package com.cn.lib.retrofit.network.subscriber

import android.content.Context

import com.cn.lib.retrofit.network.callback.ResponseCallback
import com.cn.lib.retrofit.network.exception.ApiThrowable

class RxCallbackSubscriber<T> : BaseSubscriber<T> {

    private var mCallback: ResponseCallback<T>? = null
    private var mTag: Any? = null

    constructor(tag: Any, callback: ResponseCallback<T>) {
        this.mCallback = callback
        this.mTag = tag
    }

    constructor(context: Context, tag: Any, callback: ResponseCallback<T>) : super(context) {
        this.mCallback = callback
        this.mTag = tag
    }

    override fun onStart() {
        super.onStart()
        mCallback?.run {
            onStart(mTag)
        }
    }

    override fun onNext(result: T) {
        super.onNext(result)
        mCallback?.run {
            onSuccess(mTag, result)
        }
    }

    override fun onComplete() {
        mCallback?.run {
            onCompleted(mTag)
        }
    }

    override fun onError(throwable: ApiThrowable) {
        mCallback?.run {
            onError(mTag, throwable)
        }
    }
}
