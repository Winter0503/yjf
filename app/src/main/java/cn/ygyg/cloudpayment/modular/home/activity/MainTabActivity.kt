...
class MainTabActivity : BaseIndicatorActivity() {
   ...
   override fun  getViewPagerId(): Int = R.id.view_pager
    ...
    override fun initTabsInfo(tabs: ArrayList<TabInfo>) {
        tabs.add(TabInfo(1, ResourceUtil.getString(getViewContext(), R.string.tab_home),
                R.mipmap.tab_home_normal, R.mipmap.tab_home_press, HomeFragment::class.java))
        tabs.add(TabInfo(2, ResourceUtil.getString(getViewContext(), R.string.tab_my),
                R.mipmap.tab_my_normal, R.mipmap.tab_my_press, MyFragment::class.java))
    }
